...
module.exports = (client) => {
    ...
    client.on(Events.ChannelCreate, async (channel) => {
        ...
        try {
            ...
            if (targetChannel) {
                await targetChannel.send(`**Nouveau salon détecté !** Pense à l'ajouter dans \`config.js\` :\nNom : \`${channel.name}\`\nID : \`${channel.id}\``);
            }
        } catch (error) {
    ...
    client.on(Events.GuildRoleCreate, async (role) => {
        try {
            const targetChannel = await client.channels.fetch(targetChannelId);
            if (targetChannel) {
                await targetChannel.send(`**Nouveau rôle détecté !** Pense à l'ajouter dans \`config.js\` :\nNom : \`${role.name}\`\nID : \`${role.id}\``);
            }
        } catch (error) {
